...
inline bool GetFormatFromFourCC(uint32_t fourCC, DDSTextureInfo& info) {
    ...
    switch (fourCC) {
        ...
        default:
            std::cerr << "[DDSLoader] 不支持的 FourCC: 0x" << std::hex << fourCC << std::dec << std::endl;
            return false;
    }
}
...
inline bool GetFormatFromPixelFormat(const DDSPixelFormat& ddspf, DDSTextureInfo& info) {
    ...
    std::cerr << "[DDSLoader] 不支持的未压缩格式" << std::endl;
    return false;
}
...
inline GLuint LoadDDS(const std::string& filename) {
    ...
    if (!file.is_open()) {
        std::cerr << "[DDSLoader] 无法打开文件: " << filename << std::endl;
        return 0;
    }
    ...
    if (magic != DDS_MAGIC) {
        std::cerr << "[DDSLoader] 无效的 DDS 文件: " << filename << std::endl;
        file.close();
        return 0;
    }
    ...
    if (header.size != 124) {
        std::cerr << "[DDSLoader] 无效的 DDS 头部大小: " << header.size << std::endl;
        file.close();
        return 0;
    }
    ...
    if (header.ddspf.fourCC == FOURCC_DX10) {
        ...
        std::cerr << "[DDSLoader] DX10 扩展头暂不支持" << std::endl;
        file.close();
        return 0;
    }
    ...
    if (!formatOk) {
        std::cerr << "[DDSLoader] 不支持的格式: " << filename << std::endl;
        file.close();
        return 0;
    }

    std::cout << "[DDSLoader] 加载: " << filename << std::endl;
    std::cout << "  格式: " << info.formatName << std::endl;
    std::cout << "  尺寸: " << info.width << "x" << info.height << std::endl;
    std::cout << "  Mipmap: " << info.mipMapCount << std::endl;
    ...
    for (uint32_t level = 0; level < info.mipMapCount; ++level) {
        ...
        if (!file || static_cast<uint32_t>(file.gcount()) != size) {
            std::cerr << "[DDSLoader] 读取 mipmap " << level << " 失败" << std::endl;
        ...
        if (info.isCompressed) {
            glCompressedTexImage2D(info.target, level, info.format,
                                   w, h, 0, size, data.data());
        } else {
            GLenum format = GL_RGBA;
            if (info.format == GL_RGB8) format = GL_RGB;
            else if (info.format == GL_R8) format = GL_RED;

            glTexImage2D(info.target, level, info.format,
                         w, h, 0, format, GL_UNSIGNED_BYTE, data.data());
        }
...
inline bool CheckDDSSupport() {
    ...
    if (s3tcSupported) {
        std::cout << "[DDSLoader] S3TC 压缩格式已支持" << std::endl;
    } else {
        std::cerr << "[DDSLoader] 警告: S3TC 压缩格式不支持!" << std::endl;
    }
